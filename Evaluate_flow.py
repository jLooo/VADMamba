...
def Eval(model=None, dateset=None, stage=[1], t_length=None, auc=None):
    ...
    parser.add_argument('--model_dir1', type=str, default='./', help='directory of model1')
    ...
    if model is None and task_num == 'P':  # if not training, we give a exist model and params path
        ...
        model1.cuda()
    if model is None and task_num == 'R':  # if not training, we give a exist model and params path
        model2 = vadm2(1, args.c, depths=[1, 1, 1, 1], depths_decoder=[1, 1, 1, 1], num_embeddings=10)
        try:
    ...
    auc_dir = './data/auc/{}/{}'.format(args.dataset_type, str(args.t_length))
    ...
    if task_num == 2:
        ...
        flow_net.cuda()
        if auc is not None:
            auc_npy_pred = np.load(auc_dir+'/pred_{:.8f}.npy'.format(auc))
    elif task_num == 'P':
        model1 = model[0].eval()
    elif task_num == 'R':
        model2 = model[0].eval()
    ...
    with torch.no_grad():
        for k, (imgs) in enumerate(test_batch):
            ...
            if task_num == 'P':
                ...
                if auc is not None:
                    psnr_list[videos_list[video_num].split('/')[-1]].append(psnr(auc_npy_pred[k]))  # psnr
                else:
                    psnr_list[videos_list[video_num].split('/')[-1]].append(psnr(mse_imgs_pred))  # psnr
                psnr_list_recon[videos_list[video_num].split('/')[-1]].append(psnr(mse_imgs_recon))  # psnr
        ...
        for video in sorted(videos_list):
            ...
            if task_num == 'P':
                ...
                psnr_multi_listP.extend(multi_future_frames_to_scores(np.array(normalize_score_list_gel(psnr_list[video_name]))))  # pred
                psnr_multi_listR.extend(multi_future_frames_to_scores(np.array(normalize_score_list_gel(psnr_list_recon[video_name]))))  # pred

                psnr_multi_list0.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=1))))
                psnr_multi_list01.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.1))))
                psnr_multi_list02.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.2))))
                psnr_multi_list1.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.3))))
                psnr_multi_list04.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.4))))
                psnr_multi_list2.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.5))))
                psnr_multi_list06.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.6))))
                psnr_multi_list3.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.7))))
                psnr_multi_list08.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.8))))
                psnr_multi_list4.extend(multi_future_frames_to_scores(np.array(score_sum2(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.9))))
                psnr_multi_list10.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.1))))
                psnr_multi_list20.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.2))))
                psnr_multi_list6.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.3))))
                psnr_multi_list40.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.4))))
                psnr_multi_list7.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.5))))
                psnr_multi_list60.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.6))))
                psnr_multi_list8.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.7))))
                psnr_multi_list80.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.8))))
                psnr_multi_list9.extend(multi_future_frames_to_scores(np.array(score_sum3(normalize_score_list_gel(psnr_list[video_name]),
                                                            normalize_score_list_gel(psnr_list_recon[video_name]), alpha=0.9))))
    ...
    if task_num == 'P':
        ...
        np.save(auc_dir+'/pred_{:.8f}.npy'.format(accuracy), auc_npy_pred)
        return accuracy
    elif task_num == 'R':
        print('AUC: ', accuracy * 100)
        np.save(psnr_dir+'/recon_{}.npy'.format(str(accuracy)), accuracy)
        np.save(auc_dir+'/recon_{:.8f}.npy'.format(accuracy), auc_npy_recon)
        return accuracy
    else:


        print('AUC_PRED: ', accuracyP * 100, '%', 'AUC_RECON: ', accuracyR * 100, '%', 'AUC1+1: ', accuracy * 100, '%', '\r',
              'AUC1_0.1: ', accuracy01 * 100, 'AUC1_0.2: ', accuracy02 * 100, 'AUC1_0.3: ', accuracy1 * 100, 'AUC1_0.4: ', accuracy04 * 100, '%', 'AUC1_0.5: ', accuracy2 * 100, '%',  '\r',
              'AUC1_0.6: ', accuracy06 * 100, 'AUC1_0.7: ', accuracy3 * 100, '%', 'AUC1_0.8: ', accuracy08 * 100, 'AUC1_0.9: ', accuracy4 * 100, '\r',
              'AUC0.1_1: ', accuracy10 * 100, '%', 'AUC0.2_1: ', accuracy20 * 100, '%', 'AUC0.3_1: ', accuracy6 * 100, '%', 'AUC0.4_1: ', accuracy40 * 100, '%', 'AUC0.5_1: ', accuracy7 * 100, '%', '\r',
              'AUC0.6_1: ', accuracy60 * 100, '%', 'AUC0.7_1: ', accuracy8 * 100, '%', 'AUC0.8_1: ', accuracy80 * 100, '%', 'AUC0.9_1: ', accuracy9 * 100, '%')


        all_auc = [accuracy, accuracy01, accuracy02, accuracy1, accuracy04, accuracy2, accuracy06, accuracy3, accuracy08, accuracy4, accuracy10, accuracy20, accuracy6, accuracy40, accuracy7, accuracy60, accuracy8, accuracy80, accuracy9]
