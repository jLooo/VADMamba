...
import Evaluate_frame_flow as Evaluate
...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default='/home/hxw/pythonCode/CrossSPconvUnet_4090/datasets', help='directory of data')  # /home/hxw/pythonCode/CrossSPconvUnet_4090/datasets  /home/hxw/Python/TFUnet/datasets
    ...
    # Training
    Task = 2   # 0:Pred, 1:Recon, 2:Mix, 3:No_Pred Yes_Recon  4:No_Recon Yes_Pred

    if Task == 4:
        model2_dir = os.path.join('./exp/ped2/16/log/best_model2_.pth')
    ...
    if Task == 0 or Task == 2 or Task == 4:
        for epoch in tqdm(range(args.epochs)):
            ...
            if Task == 2 or Task == 4:
                score = Evaluate.Eval(model=[model1, model2, flow_net], dateset=args.dataset_type, stage=['Pred', 'Recon'], t_length=args.t_length)
            elif Task == 0:
    ...
    if Task == 3:
        model1_dir = os.path.join('./exp/ped2/16/log/best_model1_.pth')
    ...
    if Task == 1 or Task == 2 or Task == 3:
        for epoch in tqdm(range(args.epochs)):
            ...
            for j, (imgs) in enumerate(train_batch):
                ...
                loss_ssim = 1 - calculate_ssim(restore_flow.cpu().detach().numpy(), true_flow.cpu().detach().numpy()).cuda()
            ...
            if args.method == 'pred':
                print('Loss:  {:.6f} / Recon: {:.6f} / Z_loss: {:.6f} / Diff: {:.6f} / SSIM: {:.6f}'.format(
                    loss.item(), loss_pixel_recon.item(), z2_loss.item(), loss_diff.item(), loss_ssim.item()), flush=True)
            print('----------------------------------------')

            if Task == 2 or Task == 3 or Task == 4:
                score = Evaluate.Eval(model=[model1, model2, flow_net], dateset=args.dataset_type, stage=['Pred', 'Recon'], t_length=args.t_length)
            elif Task == 1:
                score = Evaluate.Eval(model=[model2, flow_net], dateset=args.dataset_type, stage='Recon', t_length=args.t_length)
